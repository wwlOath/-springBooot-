$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'bioaitech/syslog/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.sysLog.id == null ? "bioaitech/syslog/save" : "bioaitech/syslog/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "bioaitech/syslog/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "bioaitech/syslog/info/"+id, function(r){
                vm.sysLog = r.sysLog;
            });
		},
