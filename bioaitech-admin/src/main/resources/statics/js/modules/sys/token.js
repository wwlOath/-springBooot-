	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.token.userId == null ? "sys/token/save" : "sys/token/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
		...
		getInfo: function(userId){
			$.get(baseURL + "sys/token/info/"+userId, function(r){
                vm.token = r.token;
            });
		},
